...
async function generateReport(answers, emotionLog, duration, jobTitle, interviewType) {
  const answersText = answers.map((qa, i) => `Q${i+1}: ${qa.question}\nR: ${qa.answer}`).join("\n\n");
  const emotionSummary = emotionLog.length ? Object.entries(emotionLog.reduce((acc, { emotion }) => { acc[emotion] = (acc[emotion]||0)+1; return acc; }, {})).sort((a,b)=>b[1]-a[1]).map(([e,c])=>`${e}: ${c} fois`).join(", ") : "Non disponible";
...
export default function Report() {
  ...
  const emotionEmoji = { happy: "😊", neutral: "😐", sad: "😔", angry: "😤", fearful: "😨", disgusted: "🤢", surprised: "😲" };
